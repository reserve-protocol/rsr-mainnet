...
describe('RSR contract', function () {
  ...
  describe('Transition state', async () => {
    ...
    it('should change the account weight', async () => {
      await rsr.connect(owner).siphon(addr1.address, addr1.address, addr1.address, WEIGHT_ONE)
    ...
    it('should distribute the weights correctly for the given account', async () => {
      ...
      await rsr.connect(owner).siphon(addr1.address, addr1.address, addr3.address, WEIGHT_ONE / 4)
